#!/bin/bash
# StockLens Upgrade (macOS / Linux)
# naver-stock-mcp -> stocklens-mcp v0.2
...
# [2/4] Uninstall old package (naver-stock-mcp) if present
...
if $PYTHON_CMD -m pip install --upgrade stocklens-mcp > /tmp/stocklens-upgrade.log 2>&1; then
    echo -e "      ${GREEN}✓ stocklens-mcp installed${NC}"
else
    echo -e "      ${RED}✗ Installation failed${NC}"
    echo "      Log: /tmp/stocklens-upgrade.log"
    exit 1
fi
echo ""

# [4/4] Configure Claude Desktop (auto-migrate legacy keys)
