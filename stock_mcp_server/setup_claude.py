...
def _configure_toml_target(config_path: Path, label: str, *, command: str, quiet: bool = False) -> dict:
    ...
    # 실패한다(2026-09-17 실측: search_stock 등 네 번 연속). 고객이 "GPT 에선 한 번은 꼭
    # 실패한다"고 한 그 모양이다. 우리 도구는 시세·공시 조회와 사용자 PC 에 파일 저장이
    # 전부라 자동 승인으로 둔다. 값은 auto / prompt / writes / approve 중 하나
    ...
    # 줄바꿈은 원래 파일 것을 지킨다. write_text 는 Windows 에서 \n 을 \r\n 으로 바꿔
    # 써서, LF 였던 config.toml 352줄이 통째로 CRLF 가 됐다(2026-09-17 대표 PC 실측).
    # 내용은 같아도 diff 가 전부 바뀐 것처럼 보이고, 다른 도구가 그 파일을 만지면 섞인다.
    original = config_path.read_bytes() if config_path.exists() else b""
    newline = "\r\n" if b"\r\n" in original else "\n"
    config_path.write_text(tomlkit.dumps(doc), encoding="utf-8", newline=newline)
