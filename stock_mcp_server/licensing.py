...
활성화:  stocklens-activate <라이선스-키>
저장 위치:  ~/.stocklens/license.key  (또는 STOCKLENS_HOME)
개발 우회:  환경변수 STOCKLENS_LICENSE_KEY
"""
...
def expires_on() -> date | None:
    ...
    키에 박힌 날짜가 아니라 실제로 끝나는 날(effective_expiry)이다 — 화면에 그대로
    나가는 값이라 여기서 어긋나면 안내가 거짓말이 된다."""
    k = stored_key()
    if not k:
        return None
...
def _trial_mark_paths() -> "list[Path]":
    ...
    브랜드 공용 폴더(~/.leetkit)를 쓴다. Lens 폴더(~/.stocklens)를 통째로 지우고 다시
    깔아도 여기는 남는다. 매니저가 이미 ~/.leetkit-manager 를 같은 식으로 쓴다.
    """
    return [
        _home() / "trial_started.json",
...
def _clock_seen_path() -> Path:
    return _home() / "clock_seen"
...
# GitHub의 텍스트 파일이고, 판정은 이 컴퓨터에서 이 코드가 한다. 서버가 남의 컴퓨터를
# 끄는 게 아니다.
#
...
# 대신 키를 공유받아 새로 까는 사람은 최신을 받으므로 바로 걸린다 — 한 명이 계속
# 쓰는 것보다 열 명에게 퍼지는 걸 막는 쪽이 실익이 크다.
...
def _revoked_cache_path() -> Path:
    return _home() / "revoked_cache.json"


def _load_revoked_cache() -> tuple[list[str], float]:
    try:
...
def _save_revoked_cache(ids: list[str], now: float) -> None:
    try:
...
def _fetch_revoked() -> list[str] | None:
...
def license_block_reason() -> str | None:
    """잠긴 이유. 정상이면 None.
...
def activate_cli() -> None:
    ...
    if res["valid"]:
        ...
        print("쓰고 계신 AI 앱(Claude·ChatGPT)을 완전히 종료했다가 다시 켜면 StockLens 도구를 쓸 수 있습니다.")
        sys.exit(0)
