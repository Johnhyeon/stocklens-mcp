...
설치되는 도구는 **68개**이고, 이 문서는 그중 자주 쓰는 **60개**를
다룹니다 (한국 주식 39 + 미국 주식 21). 나머지는 같은 규칙을 따르며
도구 설명에 사용법이 들어 있습니다.
...
한국투자증권 Open API 를 연결하면 국내·미국 분봉을 쓸 수 있습니다.
연결은 LeetKit Manager 의 [증권사 연결] 버튼에서 합니다 (시세 조회 전용
App Key·App Secret 만 사용, 계좌번호·주문 기능은 지원하지 않고 앞으로도
연결하지 않습니다).
...
- KIS 데이터는 항상 1분봉 원천에서 StockLens 가 세션 기준으로 집계합니다
...
- 일·주·월봉은 기존 `get_chart` / `get_us_chart` (네이버·Yahoo) 가 담당합니다.
  KIS 일봉은 수정주가 검증 전이라 제공하지 않습니다

**데이터 사용 방식** (Manager 에서 선택):
- 자동(권장): KR·US 분봉 = 연결한 증권사(KIS), 일·주·월봉 = 기존 네이버·Yahoo.
  장애가 나도 다른 공급원으로 자동 전환하지 않습니다 - 오류를 그대로
  보여주고, 원하면 source="yahoo" 를 직접 지정해 조회할 수 있습니다
- 증권사 우선: 검증된 KIS 능력을 우선
- 기본 데이터 유지: KIS 호출 0회, 연결 전과 완전히 동일하게 동작

**공급원 규칙**: 한 응답 안에서 공급원을 섞지 않습니다. 어떤 공급원이 쓰였는지는
응답 메타의 `provider` / `fallback_used` 에 항상 표시됩니다.

**미국 분봉 거래량 기준 (실측 확인)**: KIS 미국 분봉의 거래량은 상장
...
- `symbol` (KR 6자리 코드 또는 US 티커), `market` (`KR|US`), `interval` (기본 `5m`),
  ...
  `venue` (US 에서 KIS 사용 시 `NYS|NAS|AMS` 필요), `completed_only` (기본 true),
