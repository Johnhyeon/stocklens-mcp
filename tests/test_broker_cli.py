"""stocklens-broker CLI 계약 테스트 (Task 5).
...
def _assert_no_secret(payload: dict) -> None:
    ...
    assert "app_key" not in text.replace("app_key_set", "")
...
class BrokerCliHandlerTests(unittest.TestCase):
    ...
    def _handle(self, request: dict) -> dict:
        return broker_cli.handle_request(request, store=self.store)
    ...
    def test_unknown_contract_version_fails_without_mutation(self):
        self.store.save_profile(
            "real", BrokerCredentials(app_key="k", app_secret="s"))
    ...
    def test_switch_profile(self):
        self.store.save_profile(
            "real", BrokerCredentials(app_key="k", app_secret="s"))
        self.store.save_profile(
            "demo", BrokerCredentials(app_key="k2", app_secret="s2"))
...
class BrokerCliMainTests(unittest.TestCase):
    ...
    def _run_main(self, request: dict) -> tuple[int, str]:
        ...
        with patch.object(sys, "stdin", stdin), \
             ...
             patch.object(broker_cli, "_default_store", return_value=store):
