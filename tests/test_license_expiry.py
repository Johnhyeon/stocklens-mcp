...
class TestGate:
    ...
    def _isolated_home(self, tmp_path, monkeypatch):
        ...
        적는다. 홈을 안 돌려놓으면 테스트가 진짜 ~/.stocklens 에 가짜 license_id 를
        쌓는다(실제로 그랬다)."""
...
class TestClockRollback:
    ...
    def test_unwritable_state_never_locks(self, tmp_path, monkeypatch):
        """상태 파일 하나 때문에 돈 낸 사람이 잠기는 쪽이 훨씬 나쁘다.

        '없는 경로'를 주면 안 된다 — 없으면 그냥 만들어버린다. 윈도우에서는 이 경로가
        드라이브 루트로 잡혀 실제로 드라이브 최상위에 폴더가 생겼다(테스트를 돌릴 때마다).
        폴더 자리에 파일을 놓아 mkdir 이 확실히 실패하게 한다."""
        blocked = tmp_path / "blocked"
        blocked.write_text("나는 폴더가 아니다", encoding="utf-8")
        monkeypatch.setattr(L, "_home", lambda: blocked)
        self._shift(monkeypatch, -400)
        assert L._clock_turned_back(_today() + timedelta(days=5)) is False
...
class TestActivationWindow:
    ...
    def test_unwritable_marker_falls_back_to_signed_date(self, signer, tmp_path, monkeypatch):
        """적을 자리가 하나도 없으면 잠그지 않는다 — 파일 때문에 쓰던 사람이 갇히는
        쪽이 훨씬 나쁘다.
